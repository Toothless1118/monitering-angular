...
class HTTP5XXCheck(NetworkCheck):
    ...
    def _get_http_status_with_cookie(self, url, cookies):
	getres = ""
	http_status = 0
	try:
    	    getres = requests.get(url, cookies=cookies)
	    http_status = getres.status_code
    ...
    def _get_http_status_without_cookie(self, url):
	getres = ""
	http_status = 0
	try:
	...
	if login_url is not None:
		...
		auth_status = postres.status_code
		    ...
		    http_status = self._get_http_status_with_cookie(addr, cval)
		    if str(http_status) == "502":
			h502_status = 1
		    elif str(http_status) == "504":
			h504_status = 1
		elif str(http_status) == "502":
		    h502_status = 1
		elif str(http_status) == "504":
		    h504_status = 1
	    except socket.timeout, e:
	    ...
	    http_status = self._get_http_status_without_cookie(addr)
	    if str(http_status) == "502":
		h502_status = 1
	    elif str(http_status) == "504":
		h504_status = 1
	...
	self.gauge('http.http_status_502', h502_status, tags=tags_list)
	self.gauge('http.http_status_504', h504_status, tags=tags_list)
